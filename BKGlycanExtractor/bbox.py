...
@dataclass
class PDFConversionContext:
    '''
    Single class that holds conversion parameters and performs bbox conversions.
    This eliminates the need to pass many individual parameters and removes
    the need for a separate converter class.
    '''

    # using @dataclass - so no need to create __init__ (it is autogenerated)

    # Usage - pass a single datastructure into classmethod --> from_result_dict() -- which
    # will format the data and populate the below class variables appropriately.
    # This method was used in annotate_pdf.py 

    # Figure position in PDF space
    fig_pdf_x1: float
    fig_pdf_y1: float
    
    # Figure dimensions in PDF space
    fig_pdf_width: float
    fig_pdf_height: float
    
    # Figure dimensions in pixels (image space)
    pdf_px_width: int
    pdf_px_height: int
    ...
    def x_scale(self):
        ...
        return self.fig_pdf_width / self.pdf_px_width
    
    def y_scale(self):
        '''Scale factor for Y coordinate conversion.'''
        return self.fig_pdf_height / self.pdf_px_height
    
    def to_pdf_bbox(self, glycan_bbox):
        """
        Convert image bounding box to PDF bounding box.

        glycan_bbox - (x1, y1, w, h)
        """
        ...
        # Scale and translate to PDF coordinates
        pdf_x1 = self.fig_pdf_x1 + x1 * self.x_scale()
        pdf_y1 = self.fig_pdf_y1 + y1 * self.y_scale()
        pdf_x2 = (self.fig_pdf_x1 + x2 * self.x_scale() ) - 1
        pdf_y2 = (self.fig_pdf_y1 + y2 * self.y_scale() ) - 1
    ...
    @classmethod
    def from_result_dict(cls, result: dict):
        """
        Create a PDFConversionContext from a result dictionary.
        
        Args:
            result: Dictionary containing:
                - 'pdf_fig_bbox': [x1, y1, x2, y2]
                - 'pdf_fig_width': float
                - 'pdf_fig_height': float
                - 'width': int (fig pixel width)
                - 'height': int (fig pixel height)
                - 'page_width': float
                - 'page_height': float
        
        Returns:
            PDFConversionContext instance
        """

        # class instance
        return cls(
            fig_pdf_x1=result['pdf_fig_bbox'][0],
            fig_pdf_y1=result['pdf_fig_bbox'][1],
            fig_pdf_width=result['pdf_fig_width'],
            fig_pdf_height=result['pdf_fig_height'],
            pdf_px_width=result['width'],
            pdf_px_height=result['height'],
