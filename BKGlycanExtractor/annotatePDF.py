...
def checkpath(workdir):
    ...
    if os.path.isdir(check_path_pages) and os.path.isdir(check_path_annotated):
        ...
        os.makedirs(check_path_pages)
        os.makedirs(check_path_annotated)
    else:
...
def findglycans(image_path, workdir, outfilename, base_configs, file_type, log=None):
    collection_dict = {}
    base = os.getcwd()

    config = Config_Manager()

    pipeline_name = config.get('File-Pipelines', file_type, 'single_glycan')
    glycan_pipeline = config.get_pipeline(pipeline_name)

    print("figure_steps", glycan_pipeline.get_steps('figure'))
    print("glycan_steps", glycan_pipeline.get_steps('glycan'))

    # figure step
    figure_semantics = Figure_Semantics(image_path)
    for fig_step in glycan_pipeline.get_steps('figure'):
        fig_step.execute(figure_semantics)

    
    # adding glycan bounding boxes to the entire image
    entire_image = figure_semantics.image().copy()
    for i, _ in enumerate(figure_semantics.semantics['glycans']):
        print(f"Annotating glycan {i}")
        entire_image = figure_semantics.label_image(entire_image, name = f'entire_image_{i}', **dict(idx=i))
    ...
    cv2.imwrite(pages_directory, entire_image)
    ...
    for i, gly_semantics in enumerate(figure_semantics.glycans()):
        ...
        IUPAC = ''
        if mono_count - 1 == links and monoid_collection == tuple(sorted(gly_semantics.semantics['monos'].keys())):
            IUPAC = Glycan_Semantics.IUPAC(gly_semantics)
            print("IUPAC", IUPAC)

            # since we have the IUPAC sequence - we can determine the orientation of the structure
            ...
            errors.append("Mismatch in no.of links and monos")
            orientation = 'RL'
...
def parse_glycan_data(work_dict, glycan_data, results, page_num=0):
    ...
    for glycan_idx, gly_dict in glycan_data.items():
        ...
        root_id = mono_details.get('root',-1)
        result['root_id'] = root_id

        if root_id == -1:
            errors.append("Root was not found")
        else:
            result['root_confidence'] = mono_details['root_confidence']
...
def annotatePNGGlycan(work_dict):
    ...
    # switching pipelines (multi_glycan to single_glycan)
    if not glycan_data:
